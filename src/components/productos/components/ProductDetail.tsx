...
import { useProduct, parsePrice } from "../../../hooks";
import { ProductCard } from "../../common/ProductCard";

export const DEFAULT_PRODUCT_IMG = "/img/defaultProductImg.webp";

export const ProductDetail = () => {
	const { productoId } = useParams();
	const navigate = useNavigate();

	const productId = productoId ? Number.parseInt(productoId, 10) : undefined;
	...
	const handleWhatsAppClick = () => {
		...
		const formattedPrice = parsePrice(product.price).toLocaleString("es-AR", {
	...
	if (isLoading) {
		return (
			...
			>
				...
				<Text color="gray.600">Cargando producto...</Text>
			</Box>
		);
	}
	...
	const categoryLabel = product.category?.name || "Productos";
			...
			>
				...
				>
					...
					>
						<ProductCard
							to="#"
							image={product.imageUrl || DEFAULT_PRODUCT_IMG}
						...
						>
							...
							<Button
								...
								onClick={() =>
									navigate(
										categorySlug
											? `/productos?category=${categorySlug}`
											: "/productos",
									)
								}
							>
